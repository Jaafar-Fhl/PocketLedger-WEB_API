...
public class Options
{
	...
	public void Add_expense()
    {
		...
		d_base.Expenses.Add(add_new);
		d_base.SaveChanges();
		Console.WriteLine("Press 'Enter' or 'Return' to save the expense");
		Console.ReadLine();
	}

	public void List_expenses()
	{
		foreach (Expenses spending in d_base.Expenses)
    ...
    public void Edit_expenses()
	{
        ...
        foreach (Expenses spent in d_base.Expenses)
        {
        ...
        do
        {
            ...
            if (id != null)
            {
                ...
                {
					if (d_base.Expenses.Any(x => x.e_id == identity))
					{
						editable = d_base.Expenses.First(x => x.e_id == identity);
						valid_id = true;
					}
					else
					{
						Console.WriteLine("No expense with that Id exists!");
					}
				}
            ...
            case 1:
                {
					...
					d_base.Expenses.Update(editable);
                    break;
                }
            case 2:
			    {
    ...
    public void Total_spending()
    {
        ...
        Console.WriteLine($"Total spending: {total:C2} spent on {d_base.Expenses.Count()} expenses in a total of {d_base.Expenses.Max(x => x.date) - d_base.Expenses.Min(x => x.date)} days");
    }
